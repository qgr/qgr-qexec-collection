...
define(function (require) {
  ...
  function execute_query(array_map, qtree) {
    ...
    var collection = array_map[select.from];
    var result = collection.toJSON();

    // Collection executor only supports filtering/limits.
    if (select.where) {
    ...
    var result_ids = _.pluck(result, 'id');
    var filtered_models = collection.filter(function(model) {
      return _.contains(result_ids, model.id);
    });
    var filtered_collection = new collection.constructor(filtered_models);
